...
namespace panda {
	...
	public:
		...
		const static char *panda_VERSION;
		...
		const static int64_t REWARD_PERIOD = (4 * 365 * 24 * 60 * 60) / 10;
		const static int64_t REWARD_INIT_VALUE = 83333 * 1000;
	...
	class SlowTimer : public utils::Singleton<panda::SlowTimer>, public utils::Runnable {
	public:
		SlowTimer();
		~SlowTimer();
	...
	class Global : public utils::Singleton<panda::Global>, public TimerNotify {
		asio::io_service io_service_;
