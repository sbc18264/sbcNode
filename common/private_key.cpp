...
namespace panda {
	...
	bool GetKeyElement(const std::string &encode_key, PrivateKeyPrefix &prefix, SignatureType &sign_type, std::string &raw_data) {
		...
		if (buff.size() == 27 && (uint8_t)buff.at(0) == 0Xe0 && (uint8_t)buff.at(1) == 0X8b){// Address
			prefix_tmp = ADDRESS_PREFIX;
		}
		else if (buff.size() == 41 && (uint8_t)buff.at(0) == 0X9d && (uint8_t)buff.at(1) == 0X1d && (uint8_t)buff.at(2) == 0Xde){//private key
	...
	std::string PublicKey::GetEncAddress() const {
		...
		//Append prefix (panda 0XE6 0X9A 0X73 0XFF)
		//Append prefix (bu)
		str_result.push_back((char)0Xe0);
		str_result.push_back((char)0X8b);
	...
	std::string PrivateKey::GetEncPrivateKey() const {
		std::string str_result;
		//Append prefix(priv)
		str_result.push_back((char)0X9d);
		str_result.push_back((char)0X1d);
		str_result.push_back((char)0Xde);
